...
def uniformCostSearch(problem):
    ...
    currentCost = 0
    pqueue.push((problem.getStartState(), path, currentCost), currentCost)

    while pqueue.isEmpty() is False:
        currentLocation, path, currentCost = pqueue.pop()
        ...
        for node in successors:
            if node[0] not in visited:
                pqueue.push((node[0], path + [(node[1])], currentCost + node[2]), currentCost + node[2])
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while pqueue.isEmpty() is False:
        ...
        for node in successors:
            if node[0] not in visited:
                pqueue.push((node[0], path + [node[1]], currentCost + node[2]),
                            currentCost + node[2] + heuristic(node[0], problem))
    return path
